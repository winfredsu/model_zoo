...
#quantize.create_training_graph(input_graph=tf.get_default_graph(),quant_delay=0)
...
with tf.Session() as sess:
    ...
    for epoch in range(args.total_epoches):
        ...
        if epoch % args.val_evaluation_epoch == 0 and epoch >= args.warm_up_epoch:
            ...
            for j in range(args.val_img_cnt):
            ...
            if mAP > best_mAP:
                ...
                #saver_to_save.save(sess, os.path.join(FLAGS.train_dir, ckpt_name), global_step=int(__global_step))
                print('Step: ', epoch, 'Saving to ', FLAGS.train_dir)
            ...
            writer.add_summary(ake_summary('validation_statistics/total_loss', val_loss_total.average), global_step=epoch)
