...
def bottleneck(inputs,
  ...
  with tf.variable_scope(scope, 'bottleneck_v1', [inputs]) as sc:
    ...
    if depth == depth_in and stride == 1:
      # in this case no depth
      shortcut = inputs
    else:
    ...
    # use RELU6 in the final addition
  ...
  with tf.variable_scope(scope, 'resnet_v1', [inputs], reuse=reuse) as sc:
    ...
    with slim.arg_scope([slim.conv2d, bottleneck,
      ...
      with (slim.arg_scope([slim.batch_norm], is_training=is_training)
        ...
        if global_pool:
          # Global average pooling. Use AvgPool instead of reduce_mean to insert fake_quant. 
...
def my_resnet(inputs, num_classes, depth_multiplier, is_training):
  """
